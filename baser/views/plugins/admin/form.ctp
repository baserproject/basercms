...
<?php else: ?>
	<?php echo $html->link('編集する',array('action'=>'edit',$form->value('Plugin.id')),array('class'=>'btn-orange button'),null,false) ?>　
	<?php echo $html->link('削除する',array('action'=>'delete', $form->value('Plugin.id')), array('class'=>'btn-gray button'), sprintf('%s を本当に削除してもいいですか？', $form->value('Plugin.name')),false); ?>
<?php endif ?>
</div>
