...
?>
<p>インストール環境の条件をチェックしました。<br />次に進む為には、「基本必須条件」の赤いマークを全て解決する必要があります。</p>
...
<div style="margin-bottom:30px">
	...
	<ul class="section">
		<li>
			...
			<?php if (!$phpversionok): ?>
				<br /><small>ご利用のサーバーでは残念ながらBaserCMSを動作させる事はできません</small>
			<?php endif ?>
			</p>
		</li>
		<li>
	...
	<div class="section">短くスマートなURLを実現するにはApache Rewriteモジュールと.htaccessの利用許可が必要です。<br />
		...
		その場合、「.htaccess アップロード確認」の指示にしたがって「有効」にすれば問題ありません。</small>
	</div>
	<ul class="section">
		<li>
	...
	<h4>ファイルデータベース</h4>
	<div class="section">
		データベースサーバーが利用できない場合には、SQLiteやCSVファイル等ファイルベースのデータベースを利用できます。
		有効にするには、下記のフォルダへの書き込み権限が必要です
	</div>
	<ul class="section">
		<li>
    ...
    <div class="section">PHPのメモリが <?php echo $phpminimummemorylimit . " MB"; ?> より低い場合、BaserCMSの全ての機能が正常に動作しない可能性があります。<br />
        <small>サーバー環境によってはPHPのメモリ上限が取得できず「0MB」となっている場合もあります。その場合、サーバー業者等へサーバースペックを直接確認してください。</small>
    </div>
    <ul class="section">
		<li>
			...
			<?php if (!$phpmemoryok): ?>
				<br /><small>php.iniの設定変更が可能であれば、memory_limit の値を<?php echo $phpminimummemorylimit . " MB"; ?>以上に設定してください</small>
			<?php endif ?>
			</p>
		</li>
    </ul>
	...
	<div class="section">BaserCMSはセーフモードがOnの場合でも動作実績がありますが、完全にサポートしているわけではありません。
            ...
            <small>セーフモードがOnの場合、BaserCMSが内部的に利用するフォルダの生成や権限の付与に失敗する事があるので、その場合は、手動による個別対応が必要です。</small>
	</div>
	<ul class="section">
		<li>
...
<div>
    <form action='step2' method='post' style="float:left">
        <div><button class='btn-orange button' id='btncheckagain'  type='submit' ><span>再チェック</span></button></div>
    </form>
</div>

<div>
    <form action='step3' method='post'>
        <div><button class='btn-red button' <?php if (!$blRequirementsMet): ?>style="display:none" disabled='disabled' <?php endif ?> id='btnnext' type='submit' ><span>次のステップへ</span></button></div>
    </form>
</div>