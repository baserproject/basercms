...
<strong>WEBサイトのバージョン</strong>：<?php echo $siteVer ?>
</p>
<?php if($baserVer != $siteVer): ?>
    <p>WEBサイトのアップデートを実行してBaserCMSのアップデートを完了させてください。</p>
    <?php if($scriptNum): ?><p>アップデートプログラムが <strong><?php echo $scriptNum ?> 個</strong> あります。<br />
        <strong>実行する前には必ずデータベースをバックアップしておいてください。</strong></p>
    <p><?php $baser->link('≫ バックアップはこちらから','/admin/site_configs/form') ?></p>
    <?php endif ?>
    <?php echo $form->create(array('action'=>'update')) ?>
