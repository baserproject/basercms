...
<div id="installations">
    ...
    <div>
        <p>
            データベースサーバーの場合は、データベース情報を入力し次に進む前に接続テストを実行して下さい。<br />
            <strong>CSV以外の場合は、データベースが存在し中身が空である必要があります。</strong>
        </p>
    </div>
    ...
    <div>
        <form action="step3" method="post" name='dbsettings' id="dbsettings">
            <ul>
                <li id="dbType">
                    <?php echo $form->label('installation.dbType', 'データベースタイプ');?><br />
                    <?php echo $form->select('installation.dbType',$dbsource , $defaultdb, array(),false); ?>
                </li>
                <li id="dbHost">
                    <?php echo @$form->label('installation/dbHost', 'データベースホスト名');?><br />
                    <?php echo @$form->text('installation.dbHost',array('maxlength'=>'300','size'=>45, 'value'=>$dbHost)); ?>
                </li>
                <li id="dbUser" class="clearfix">
                    <label>ログイン情報</label><br />
                    <div class="float-left">
                        <?php echo @$form->text('installation.dbUsername',array('maxlength'=>'100')); ?><br />
                        <small>ユーザー名</small>
                    </div>
                    <div class="float-left">
                        <?php echo @$form->text('installation.dbPassword',array('maxlength'=>'100','type'=>'password')); ?><br />
                        <small>パスワード</small>
                    </div>
                </li>
                <li id="dbInfo" class="clearfix">
                    <label>データベース情報</label><br />
                    <div class="float-left">
                        <?php echo @$form->text('installation.dbDBName',array('maxlength'=>'100','value'=>$dbDBName)); ?><br />
                        <small>データベース名</small>
                    </div>
                    <div style="display:none">
                        <?php echo @$form->text('installation.dbPrefix',array('maxlength'=>'50','value'=>$dbPrefix)); ?>
                        <small>プレフィックス</small>
                    </div>
                    <div class="float-left">
                    <?php echo @$form->text('installation.dbPort',array('maxlength'=>'5','size'=>5,'value'=>$dbPort)); ?><br />
                    <small>ポート</small>
                    </div>
    ...
    <div class="clearfix">
        ...
        <div class="float-left">
            <button class='btn-orange button' name="checkdb" type='submit' id='checkdb'>
                <span>接続テスト</span>
            </button>
        </div>

        <button class='btn-red button' name="btnnext" id='btnnext' type='button' <?php if (!isset($blDBSettingsOK) || !$blDBSettingsOK): ?> disabled='disabled' <?php endif ?>>
            <span>次のステップへ</span>
        </button>
    </div>
