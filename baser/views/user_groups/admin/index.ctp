...
<div class="help-box corner10 display-none" id="helpAdminBody">
	...
	<ul>
		<li>アクセス制限をかけるには各ユーザーグループの「権限」ボタンをクリックしておこないます。</li>
...
<?php if(!empty($listDatas)): ?>
	...
	<tr<?php echo $class; ?>>
		...
		<?php if($listData['UserGroup']['name']!='admins'): ?>
			<?php $baser->link('削除', 
					array('action' => 'delete', $listData['UserGroup']['id']), array('class' => 'btn-gray-s button-s'),
					sprintf('%s を本当に削除してもいいですか？\n\n削除する場合、関連するユーザーは削除されませんが、関連するアクセス制限設定は全て削除されます。\n※ 関連するユーザーは管理者グループに所属する事になります。', $listData['UserGroup']['title']),
					false); ?>
		<?php endif ?>
		</td>
		...
		<td><?php echo $timeEx->format('y-m-d',$listData['UserGroup']['created']) ?><br />
			<?php echo $timeEx->format('y-m-d',$listData['UserGroup']['modified']) ?></td>
	</tr>
		<?php $count++; ?>
	<?php endforeach; ?>
<?php else: ?>
