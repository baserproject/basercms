...
<?php if($data['name']==$baser->siteConfig['theme']): ?>
...
<?php endif ?>		
		<?php $baser->link($baser->getImg('admin/icn_tool_manage.png', array('width' => 24, 'height' => 24, 'alt' => '管理', 'class' => 'btn')), array('controller'=>'theme_files','action' => 'index', $data['name']), array('title' => '管理')) ?>
<?php if($data['name'] != 'core'): ?>
		<?php $baser->link($baser->getImg('admin/icn_tool_edit.png', array('width' => 24, 'height' => 24, 'alt' => '編集', 'class' => 'btn')), array('action' => 'edit', $data['name']), array('title' => '編集')) ?>
		<?php $baser->link($baser->getImg('admin/icn_tool_copy.png', array('width' => 24, 'height' => 24, 'alt' => 'コピー', 'class' => 'btn')), array('action' => 'ajax_copy', $data['name']), array('title' => 'コピー', 'class' => 'btn-copy')) ?>
<?php endif ?>
...
<?php endif ?>
	...
	<td style="width:220px">
		...
		<a href="<?php echo $html->webroot('/themed/'.$data['name'].'/screenshot.png') ?>" rel="colorbox" class="test">
			<?php $baser->img('/themed/'.$data['name'].'/screenshot.png',array(
						'alt'=>$data['title'],
						'width'=>'80px',
	...
	<td><?php if(!empty($data['url']) && !empty($data['author'])): ?>
		<?php $baser->link($data['author'],$data['url'],array('target'=>'_blank')) ?>
		<?php else: ?>
