...
<?php endif ?>
<?php echo $formEx->hidden('ThemeFolder.parent') ?>
<?php echo $formEx->hidden('ThemeFolder.pastname') ?>
	...
	<tr>
		...
		<td class="col-input">
			<?php if($this->action != 'admin_view_folder'): ?>
				<?php echo $formEx->text('ThemeFolder.name', array('size'=>40,'maxlength'=>255)) ?>
			...
			<?php else: ?>
				<?php echo $formEx->text('ThemeFolder.name', array('size'=>40,'readonly'=>'readonly')) ?>
			<?php endif ?>
		</td>
	</tr>
</table>
...
<?php else: ?>
	<?php if($theme == 'core'): ?>
		<?php $baser->link('現在のテーマにコピー',array('action'=>'copy_folder_to_theme',$theme, $plugin, $type , $path) , array('class'=>'btn-red button'),'本当に現在のテーマ「'.Inflector::camelize($theme).'」にコピーしてもいいですか？\n既に存在するファイルは上書きされます。'); ?>
	<?php endif ?>
<?php endif ?>
</div>
