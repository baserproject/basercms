		...
		if (strpos($url, 'css/') !== false || strpos($url, 'js/') !== false || strpos($url, 'img/') !== false) {
			...
			foreach ($assets as $type => $contentType) {
				if ($type === $ext) {
					...
					// >>> 2009/11/04 MODIFIED egashira
					// pluginsのvendorsディレクトリ内のcss/imagesフォルダを読み込むように変更
					// ※ 他のvendorsディレクトリはcss/imagesを読み込むようになっているが、plugins内のvendorsだけ読み込まれない為
					// また、js / css 内のimgフォルダも正しく読み込まれるようにした。
					/*if ($type === 'css' || $type === 'js') {
						$pos = strpos($url, $type . '/');
					} else {
						$pos = strpos($url, 'img/');
