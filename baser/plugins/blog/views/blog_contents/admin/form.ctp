...
<?php else: ?>
	<?php echo $html->link('編集する',array('action'=>'edit',$form->value('BlogContent.id')),array('class'=>'btn-orange button'),null,false) ?>　
	<?php echo $html->link('削除する',array('action'=>'delete', $form->value('BlogContent.id')), array('class'=>'btn-gray button'), sprintf('%s を本当に削除してもいいですか？', $form->value('BlogContent.name')),false); ?>
<?php endif ?>
</div>
