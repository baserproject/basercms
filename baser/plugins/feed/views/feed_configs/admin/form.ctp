...
<table cellpadding="0" cellspacing="0" class="admin-row-table-01 slide-body" id="formOptionBody">
	<tr>
		...
		<td class="col-input">
			...
			<?php echo $html->image('img_icon_help_admin.gif', array('id' => 'helpFeedTitleIndex', 'class' => 'help', 'alt' => 'ヘルプ')) ?>
	...
	<tr>
		...
		<td class="col-input">
			...
			<?php echo $html->image('img_icon_help_admin.gif', array('id' => 'helpCategoryIndex', 'class' => 'help', 'alt' => 'ヘルプ')) ?>
...
<table cellpadding="0" cellspacing="0" class="admin-col-table-01">
	...
	<tr>
		<td class="operation-button">
			...
			<?php $baser->link('編集', 
					array('controller' => 'feed_details', 'action' => 'edit', $formEx->value('FeedConfig.id'), $feedDetail['id']),
					array('class'=>'btn-orange-s button-s'), null, false) ?>
			<?php $baser->link('削除', 
					array('controller' => 'feed_details', 'action' => 'delete', $formEx->value('FeedConfig.id'), $feedDetail['id']),
					array('class' => 'btn-gray-s button-s'),
					sprintf('%s を本当に削除してもいいですか？', $feedDetail['name']), false); ?>
		</td>
		<td>
		...
		<td><?php echo $timeEx->format('y-m-d', $feedDetail['created']) ?><br />
			<?php echo $timeEx->format('y-m-d', $feedDetail['modified']) ?></td>
	</tr>
		<?php endforeach; ?>
	<?php else: ?>
	<tr>
