...
<table cellpadding="0" cellspacing="0" class="admin-col-table-01" id="TableFeedConfigs">
	...
	<?php if(!empty($feedConfigs)): ?>
		...
		<?php foreach($feedConfigs as $feedConfig): ?>
			...
			<?php endif; ?>
	<tr<?php echo $class; ?>>
		<td class="operation-button"><?php $baser->link('確認','/admin/feed/feed_configs/preview/'.$feedConfig['FeedConfig']['id'],array("target"=>"_blank",'class'=>'btn-green-s button-s')) ?>
			<?php $baser->link('編集',array('action'=>'edit', $feedConfig['FeedConfig']['id']),array('class'=>'btn-orange-s button-s'),null,false) ?>
			<?php $baser->link('削除', array('action'=>'delete', $feedConfig['FeedConfig']['id']), array('class'=>'btn-gray-s button-s'), sprintf('%s を本当に削除してもいいですか？', $feedConfig['FeedConfig']['name']),false); ?></td>
