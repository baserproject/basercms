		...
		<?php if(!empty($globalMenus)): ?>
			<?php foreach($globalMenus as $key => $globalMenu): ?>
				...
				<?php if($globalMenu['GlobalMenu']['status']): ?>
					...
					<?php endif ?>
					<?php if(!Configure::read('BcRequest.agent') && $this->request->base == '/index.php' && $globalMenu['GlobalMenu']['link'] == '/'): ?>
	<?php /* PC版トップページ */ ?>
	<li<?php echo $class ?>><?php echo str_replace('/index.php','',$this->BcBaser->link($globalMenu['GlobalMenu']['name'],$globalMenu['GlobalMenu']['link'])) ?></li>
					<?php else: ?>
	<li<?php echo $class ?>>
