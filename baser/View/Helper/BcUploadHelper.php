 ...
 */
	function fileLink($fieldName, $options = array()) {
		...
		$basePath = $this->request->base.DS.'files'.DS.$model->actsAs['BcUpload']['saveDir'].DS;
		...
		if(is_array($value)) {
			if(empty($value['session_key']) && empty($value['name'])) {
				...
				if(isset($value['session_key'])) {
					...
					$basePath = $this->request->base.DS.'uploads'.DS.'tmp'.DS;
				}else{
					return false;
				}
			}
 ...
 */
	function uploadImage($fieldName, $fileName, $options = array()) {
		...
		if($tmp) {
			$link = false;
			$fileUrl = $this->request->base.'/uploads/tmp/';
			if($imgsize) {
				$fileUrl .= $imgsize.'/';
			}
		}
