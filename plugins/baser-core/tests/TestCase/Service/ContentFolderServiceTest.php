...
class ContentFolderServiceTest extends BcTestCase
{
    ...
    {
        $content = $this->Contents->get(1)->toArray();
        $data = [
            'ContentFolder' => [
                'id' => '1',
        ...
        $result = $this->ContentFolderService->update($contentFolder, $data["ContentFolder"]);
        $errors = $result->content->getErrors();
        $this->assertTrue($result->content->hasErrors());
        // $contentFolders = $this->ContentFolderService->getIndex();
