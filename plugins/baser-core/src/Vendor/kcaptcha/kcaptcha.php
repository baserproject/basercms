...
# Copyright by Kruglov Sergei, 2006, 2007, 2008, 2011, 2016, 2023
	...
	function __construct(){ // new PHP declaration
		...
		do{
			...
			while(true){
				$this->keystring='';
				for($i=0;$i<$length;$i++){
					$this->keystring.=substr($allowed_symbols, mt_rand(0,strlen($allowed_symbols)-1), 1);
				}
			...
			for($i=0;$i<$length;$i++){
							...
							for($sx=$m['start']-1;$sx<$m['end'];$sx+=1){
								...
								if($opacity<127){
									...
									for($px=min($left,$width-1);$px>$left-200 && $px>=0;$px-=1){
						        		$color=imagecolorat($img, $px, (int) $py) & 0xff;
				...
				imagecopy($img, $font, $x-$shift, (int) $y, $m['start'], 1, $m['end']-$m['start'], $fontfile_height);
