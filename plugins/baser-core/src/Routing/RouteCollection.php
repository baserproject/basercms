    ...
    public function match(array $url, array $context): string
    {
        ...
        foreach ($this->_getNames($url) as $name) {
            ...
            foreach ($this->_routeTable[$name] as $route) {
                ...
                if ($match) {
                    // CUSTOMIZE 2021/08/21 ryuring
                    // URLの末尾のスラッシュを強制的に削除する仕様になっているため
                    // 削除しないように改修
                    // スラッシュを削除すると階層が変わってしまい、CSS等のリンク切れを引き起こしてしまう為
                    // >>>
                    // return strlen($match) > 1 ? trim($match, '/') : $match;
                    // ---
					if(get_class($route) === 'BaserCore\Routing\Route\BcContentsRoute') {
						return ltrim($match, '/');
					} else {
						return strlen($match) > 1 ? trim($match, '/') : $match;
					}
