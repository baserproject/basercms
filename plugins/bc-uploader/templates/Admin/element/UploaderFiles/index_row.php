...
<tr<?php echo $class; ?> id="selectedFile<?php echo $file['UploaderFile']['id'] ?>">
  ...
  <td
    class="img bca-table-listup__tbody-td"><?php echo $this->Uploader->file($file, ['size' => 'small', 'alt' => h($file['UploaderFile']['alt']), 'style' => 'width:80px']) ?></td>
  ...
  <td
    class="user-name bca-table-listup__tbody-td"><?php echo h($this->BcText->arrayValue($file['UploaderFile']['user_id'], $users)) ?></td>
  ...
  <?php if (!$listId): ?>
    <td class="row-tools bca-table-listup__tbody-td bca-table-listup__tbody-td--actions" style="width:15%">
      ...
      <?php $this->BcBaser->link('', ['action' => 'delete', $file['UploaderFile']['id']], ['title' => __d('baser', '削除'), 'class' => 'bca-btn-icon', 'data-bca-btn-type' => 'delete', 'data-bca-btn-size' => 'lg']) ?>
    </td>
  <?php endif ?>
</tr>
