 ...
 * @var BcAppView $this
 */
$this->BcBaser->js('admin/theme_files/form_folder.bundle', false);
...
<div class="section">
  <table id="FormTable" class="form-table bca-form-table">
    <tr>
      <th
        class="col-head bca-form-table__label"><?php echo $this->BcAdminForm->label('ThemeFolder.name', __d('baser', 'フォルダ名')) ?>
      ...
      <td class="col-input bca-form-table__input">
        <?php if ($this->request->action != 'admin_view_folder'): ?>
          ...
          <?php echo $this->BcAdminForm->error('ThemeFolder.name') ?>
        <?php else: ?>
...
<div class="submit bca-actions">
  <?php if ($this->request->action == 'admin_add_folder'): ?>
    <div class="bca-actions__main">
      <?php echo $this->BcAdminForm->button(__d('baser', '保存'), ['div' => false, 'class' => 'button bca-btn bca-actions__item', 'data-bca-btn-type' => 'save', 'data-bca-btn-size' => 'lg', 'data-bca-btn-width' => 'lg', 'id' => 'BtnSave']) ?>
    </div>
  <?php elseif ($this->request->action == 'admin_edit_folder'): ?>
    <?php if ($isWritable): ?>
      ...
      <div class="bca-actions__sub">
        <?php $this->BcBaser->link(__d('baser', '削除'), array_merge(['action' => 'del', $theme, $type], $params), ['class' => 'bca-submit-token button bca-btn bca-actions__item', 'data-bca-btn-type' => 'delete', 'data-bca-btn-size' => 'sm'], sprintf(__d('baser', '%s を本当に削除してもいいですか？'), $this->BcAdminForm->getSourceValue('ThemeFolder.name')), false) ?>
      </div>
    <?php endif ?>
  <?php else: ?>
    <?php if (!$safeModeOn): ?>
      <?php if ($theme == 'core'): ?>
        <?php $this->BcBaser->link(__d('baser', '現在のテーマにコピー'), array_merge(['action' => 'copy_folder_to_theme', $theme, $plugin, $type], $params), ['class' => 'bca-submit-token button bca-btn'], sprintf(__d('baser', '本当に現在のテーマ「%s」にコピーしてもいいですか？\n既に存在するファイルは上書きされます。'), Inflector::camelize($siteConfig['theme']))); ?>
      <?php endif ?>
    <?php else: ?>
...
<?php if ($this->request->action == 'admin_add_folder' || $this->request->action == 'admin_edit_folder'): ?>
  <?php $this->BcBaser->link(__d('baser', '一覧に戻る'), array_merge(['action' => 'index', $theme, $plugin, $type], explode('/', $path)), ['class' => 'button bca-btn', 'data-bca-btn-type' => 'back-to-list']); ?>
<?php else: ?>
