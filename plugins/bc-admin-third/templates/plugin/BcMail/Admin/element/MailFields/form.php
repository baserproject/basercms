      ...
      <td class="col-input bca-form-table__input">
        ...
        <div class="bca-helptext">
          <ul>
            ...
            <li><?php echo __d('baser_core', '正規表現チェック：利用するには、オプション項目に、regex と正規表現を次の形式のように |（パイプ） 区切りで入力します。「regex|正規表現の内容」 入力した正規表現は以下の書式で実行されます。「/\A正規表現の内容\z/us」') ?>　<br><strong><?php echo __d('baser_core', '（入力例）') ?></strong>
              <ul>
