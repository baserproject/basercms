...
<tr id="RowMetaBcCcWysiwyg" class="bca-row-meta">
    <th class="col-head bca-form-table__label">
        <?php echo $this->BcAdminForm->label('', __d('baser_core', 'Wysiwyg エディタ設定')) ?>
    </th>
    <td class="col-input bca-form-table__input">
        <?php echo $this->BcAdminForm->label('meta.BcCcWysiwyg.width', __d('baser_core', '横幅')) ?>&nbsp;&nbsp;
        <?php echo $this->BcAdminForm->control('meta.BcCcWysiwyg.width', ['type' => 'text', 'size' => 10, 'placeholder' => '100%']) ?>　　
        <?php echo $this->BcAdminForm->label('meta.BcCcWysiwyg.height', __d('baser_core', '高さ')) ?>&nbsp;&nbsp;
        <?php echo $this->BcAdminForm->control('meta.BcCcWysiwyg.height', ['type' => 'text', 'size' => 10, 'placeholder' => '200px']) ?>
        <i class="bca-icon--question-circle bca-help"></i>　　
        <div class="bca-helptext">
          <?php echo __d('baser_core', 'ピクセル（px）、または、パーセンテージ（%）で単位も含めて入力してください。') ?>
        </div>
        <?php echo $this->BcAdminForm->label('meta.BcCcWysiwyg.editor_tool_type', __d('baser_core', 'エディタのタイプ')) ?>&nbsp;&nbsp;
        <?php echo $this->BcAdminForm->control('meta.BcCcWysiwyg.editor_tool_type', ['type' => 'select', 'options' => [
            'simple' => __d('baser_core', 'シンプル'),
            'normal' => __d('baser_core', 'ノーマル')
        ]]) ?>
