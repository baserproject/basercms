...
namespace BcContentLink\Controller\Api\Admin;
 ...
 * リンク コントローラー
 */
class ContentLinksController extends BcAdminApiController
{
    ...
    public function view(ContentLinksServiceInterface $service, $id)
    {
        ...
        try {
            $contentLink = $service->get($id, $this->getRequest()->getQueryParams());
        } catch (RecordNotFoundException $e) {
            $this->setResponse($this->response->withStatus(404));
            $message = __d('baser_core', 'データが見つかりません。');
        } catch (Throwable $e) {
