	...
	protected function seek($selector, &$ret, $parent_cmd, $lowercase = false)
	{
		...
		foreach($nodes as $node) {
			...
			if ($pass && $id !== '' && isset($node->attr['id'])) {
				...
				// >>> CUSTOMIZE ADD kato 2020/07/15 - php5.3環境下で動作するようにexplode(~)を一時変数に格納する
				$arr_node_id = explode(' ', trim($node->attr['id']));
				$node_id = $arr_node_id[0];
				// <<<
...
class simple_html_dom
{
	...
	protected function parse_attr($node, $name, &$space)
	{
		...
		// html whitespace.
		// >> CUSTOMIZE GONDOH 2016/02/09 - baserCMSのメールフォームのvalue値が自動的に改行を削除されてしまう問題の修正
		if ($name != "value") {
