...
class BlogPostsToolTest extends BcTestCase
{
    ...
    public function testAddBlogPostWithBase64EyeCatch()
    {
        ...
        $base64Data = $this->eyeCatchBase64();
    ...
    /**
     * editBlogPost に正当な data: URI を渡した場合、アイキャッチが差し替わることを
     * 確認するテスト
     *
     * eyeCatch の分岐は「未指定＝変更しない」「空文字列＝削除」「不正値＝エラー」
     * 「アップロード失敗＝エラー」と成功の5経路がある。異常系にテストを足した際、
     * 主経路である成功だけが未検証だったため、条件を書き換えて正当な値まで
     * 弾いてしまっても気づけない状態だった。
     */
    public function testEditBlogPostWithValidEyeCatchReplacesExisting()
    {
        $this->makeBlogPostWithEyeCatch('existing.jpg');
        ...
        $result = $this->BlogPostsTool->editBlogPost(
            ...
            null,
            null,
            $this->eyeCatchBase64() // eyeCatch（正当な data: URI）
        );

        // 成功レスポンスであることを確認（エラー応答には title が含まれない）
        $this->assertArrayHasKey('title', $result);

        // 元のファイル名から差し替わっていることを確認
        $this->assertNotEmpty($result['eye_catch']);
        $this->assertNotEquals('existing.jpg', $result['eye_catch']);

        // 実ファイルが生成されていることを確認
        $filePath = WWW_ROOT . 'files' . DS . 'blog' . DS . '1' . DS . 'blog_posts' . DS . $result['eye_catch'];
        $this->assertFileExists($filePath);
