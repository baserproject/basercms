 ...
 */
	public function testAutoIgnoreBadInsertData() {
		...
		$result = CakeText::insert('(:foo > :bar || :fale!)', $data, ['clean' => 'text']);
...
TEXT;
		...
		$result = CakeText::wrap($text, ['width' => 20, 'wordWrap' => false]);
 ...
 */
	public function testWrapIndent() {
		$text = 'This is the song that never ends. This is the song that never ends. This is the song that never ends.';
		$result = CakeText::wrap($text, ['width' => 33, 'indent' => "\t", 'indentAt' => 1]);
		$expected = <<<TEXT
This is the song that never ends.
	This is the song that never ends.
	This is the song that never ends.
 ...
 */
	public function testTruncate() {
		...
		$this->assertSame($this->Text->truncate($text1, 15, ['html' => true]), 'The quick brow' . $elipsis);
		$this->assertSame($this->Text->truncate($text1, 15, ['exact' => false, 'html' => true]), 'The quick' . $elipsis);
		$this->assertSame($this->Text->truncate($text2, 10, ['html' => true]), 'Heiz&ouml;lr&uuml;c' . $elipsis);
		$this->assertSame($this->Text->truncate($text2, 10, ['exact' => false, 'html' => true]), $elipsis);
		$this->assertSame($this->Text->truncate($text3, 20, ['html' => true]), '<b>&copy; 2005-2007, Cake S' . $elipsis . '</b>');
		$this->assertSame($this->Text->truncate($text4, 15, ['html' => true]), '<img src="mypic.jpg"> This image ta' . $elipsis);
		$this->assertSame($this->Text->truncate($text4, 45, ['html' => true]), '<img src="mypic.jpg"> This image tag is not XHTML conform!<br><hr/><b>But the' . $elipsis . '</b>');
...
<p><span style="font-size: medium;"><a>... </a></span></p>';
		...
		$result = $this->Text->truncate($text1, 31, [
			'ending' => '.',
			'exact' => false,
		]);
 ...
 */
	public function testHighlightHtml() {
		...
		$this->assertEquals($this->Text->highlight($text3, ['strong', 'what'], $options), $text3);
