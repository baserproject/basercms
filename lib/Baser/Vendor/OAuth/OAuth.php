...
class OAuthSignatureMethod_RSA_SHA1 extends OAuthSignatureMethod {
	...
	function fetch_public_cert(&$request) {
		...
		die("fetch_public_cert not implemented");
	}
	/**
	 * @access	protected
	 */
	function fetch_private_cert(&$request) {
		...
		//
		// either way should return a string representation of the certificate
		die("fetch_private_cert not implemented");
	}
	/**
	 * @access	public
	 */
...
class OAuthRequest {
		...
		if (!$parameters) {
			...
			if ($http_method == "POST"
				...
				$post_data = OAuthUtil::parse_parameters(
						file_get_contents($this->{$POST_INPUT})
				);
	...
	function &from_consumer_and_token(&$consumer, $token, $http_method, $http_url, $parameters=NULL) {
		...
		@$parameters or $parameters = array();
		$defaults = array("oauth_version" => '1.0', //$OAuthRequest->version,
...
class OAuthServer {
	...
	function get_version(&$request) {
		...
		if ($version && $version != $this->version) {
			die("OAuth version '$version' not supported");
		}
		return $version;
	}
	...
	function get_consumer(&$request) {
		...
		if (!$consumer_key) {
			die("Invalid consumer key");
		}

		$consumer = $this->data_store->lookup_consumer($consumer_key);
		if (!$consumer) {
			die("Invalid consumer");
		}
	...
	function get_token(&$request, $consumer, $token_type="access") {
		...
		if (!$token) {
			die("Invalid $token_type token: $token_field");
		}
		return $token;
	}
...
/**
 * PHP4では hash_hmac をサポートしていないのでここで定義
 *
 * @param string $key
 * @param string $data
 * @return string
 */
function hash_hmac_sha1($key, $data)
{
	$blocksize = 64;
	$hashfunc = 'sha1';
...
if (!function_exists('hash_hmac')) {
    ...
    {
		if($algo=='sha1'){
			return hash_hmac_sha1($key, $data);
		}else{
			die($algo.' is no supported.');
		}
    }
}
