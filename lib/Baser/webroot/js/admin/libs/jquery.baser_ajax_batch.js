  ...
  $.baserAjaxBatch = {
    ...
    initList: function () {
      ...
      $($.baserAjaxBatch.config.executeButton).click(function () {
        ...
        if (config.methods[method] != null && !confirm(config.methods[method].confirm)) {
          return false
        }
        ...
        $.bcToken.check(function () {
          ...
          $.ajax({
            ...
            success: function (result) {
              ...
              form.remove()
              if (result) {
                if (config.methods[result].result != null) {
            ...
            error: function (xhr, textStatus, errorThrown) {
              ...
              $(config.loader).hide()
              form.remove()
              $(config.alertBox).html('一括処理に失敗しました。(' + xhr.status + ')' + errorMessage)
              $(config.alertBox).fadeIn(500)
            }
          })
        }, { hideLoader: false })
