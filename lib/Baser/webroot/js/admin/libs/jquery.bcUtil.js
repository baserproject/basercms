...
(function ($) {
  $.bcUtil = {
    ...
    showLoader: function (type, selector, key) {
      if (type == null || (type !== 'none' && selector == null)) {
        type = 'over'
      }
      var img
      switch (type) {
        ...
        case 'inner':
          ...
          img = $('<img>').attr('src', $.baseUrl + '/img/admin/ajax-loader.gif')
          div.html(img)
          $(selector).html(div)
          break
        case 'after':
          img = $('<img>').attr('src', $.baseUrl + '/img/admin/ajax-loader-s.gif').attr('id', key)
          $(selector).after(img)
          break
        case 'target':
          $(selector).show()
