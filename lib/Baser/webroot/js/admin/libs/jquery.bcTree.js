					...
					if($.bcTree.settings[data.contentType] == undefined || !$.bcTree.settings[data.contentType].editDisabled) {
						if (!data.alias) {
							if($.bcTree.settings[data.contentType] == undefined) {
								$.bcTree.openUrl($.bcTree.createLink($.baseUrl + '/admin/contents/edit', data.contentId, data.contentParentId, data.contentEntityId));
							} else {
				...
				$.bcToken.check(function(){
					$.ajax({
						...
						success: function (result) {
							...
							$.ajax($.baseUrl + '/admin/contents/ajax_get_full_url/' + data.contentId, {type: 'GET'}).done(function(result) {
