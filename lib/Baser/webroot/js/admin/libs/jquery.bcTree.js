...
(function ($) {
  $.bcTree = {
     ...
     * ツリーを読み込む
     *
     * TODO: functions.js で定義されたグローバル汚染関数 getNowDateTime を使用
     */
    load: function () {
      if (!$.bcTree._inited) {
        return
     ...
     * ツリーを初期化する
     *
     * TODO: functions.js で定義されたグローバル汚染関数 p を使用
     */
    _init: function () {
      ...
      $.bcTree.treeDom.bind('dblclick.jstree', function (event) {
        ...
        if (data.type === 'default' || data.alias) {
          ...
          if ($.bcTree.settings[data.contentType] == null || !$.bcTree.settings[data.contentType].editDisabled) {
            if (!data.alias) {
              if ($.bcTree.settings[data.contentType] == null) {
        ...
        'core': {
          ...
          'check_callback': function (operation, node, nodeParent, nodePosition, more) {
            ...
            if (operation === 'move_node') {
              if (nodeParent.type === 'folder' && !nodeParent.data.jstree.alias && !node.data.jstree.contentSiteRoot) {
                $.bcTree.dropTarget = nodeParent
          ...
          'items': function (node) {
            ...
            if ($.bcTree.settings[data.contentType] == null) {
              ...
              if (node.data.jstree.alias) {
                ...
                editDisabled = $.bcTree.settings[data.contentType].editDisabled
                manageDisabled = $.bcTree.settings[data.contentType].manageDisabled
              }
            }
            ...
            // - 公開されている
            // - URLがある
            // TODO: data.statusの型を調べる
            if (data.status && data.contentFullUrl && !$.bcTree.contextmenuAddOnly && mode === 'index') {
              $.extend(true, menu, {
                'view': {
                  label: '確認',
                      ...
                      $.bcToken.check(function () {
                        $.ajax({
                          ...
                          error: function (xhr, textStatus, errorThrown) {
                            $.bcUtil.showAjaxError('公開状態の変更に失敗しました。', xhr, errorThrown)
                          },
                          complete: function () {
                            $.bcUtil.hideLoader()
                          }
            ...
            // - フォルダーでない
            // - コピー機能サポート
            if (!editDisabled && !$.bcTree.contextmenuAddOnly && data.contentType !== 'ContentFolder' && !data.alias && $.bcTree.settings[data.contentType]['url']['copy'] != null && mode === 'index') {
              $.extend(true, menu, {
                'copy': {
                  label: 'コピー',
    ...
    refreshTree: function () {
      ...
      $('li.jstree-node').each(function (i) {
        ...
        // TODO: node.data.jstree.statusの型を調べる
        if (!node.data.jstree.status) {
          if (i % 2 === 0) {
            $(this).find('div.jstree-wholerow').each(function () {
      ...
      $.bcToken.check(function () {
        $.ajax({
          ...
          success: function (result) {
            ...
            // TODO: $.bcTree.jsTree.get_json().length が数値型かどうか調べる
            if (!$.bcTree.jsTree.get_json('#', { flat: true }).length) {
              $('#DataList').html('<div class="tree-empty">ゴミ箱は空です</div>')
            }
