...
(function($){
	 ...
	 */
		load: function () {
			...
			if(mode == 'index') {
				...
				url = $.baseUrl+'/admin/contents/index/site_id:' + siteId + '/list_type:1';
			} else if(mode == 'trash') {
		...
		_init: function() {
			...
			$.bcTree.treeDom.bind("dblclick.jstree", function (event) {
				...
				if(data.type == 'default' || data.alias) {
					if(!$.bcTree.settings[data.contentType].editDisabled) {
						if (!data.alias) {
			...
			$.bcTree.treeDom.jstree({
				...
				"contextmenu": {
					...
					"items": function (node) {
						...
						// - サイトルートではない
						// - 関連データではない
						if(!editDisabled && $.bcTree.isPublishByParents(node) && !data.contentSiteRoot && mode == 'index' && !$.bcTree.contextmenuAddOnly && !data.related) {
							if (data.status == "0") {
								$.extend(true, menu, {
									"publish": {
										label: "公開",
						...
						// - 管理機能サポート
						// - エイリアスではない
						if(!manageDisabled && !$.bcTree.contextmenuAddOnly && $.bcTree.settings[data.contentType]['routes']['manage'] !== undefined && mode == 'index' && !data.alias) {
							$.extend(true, menu, {
								"manage": {
									label: "管理",
						...
						// - フォルダーでない
						// - コピー機能サポート
						if(!editDisabled && !$.bcTree.contextmenuAddOnly && data.contentType != 'ContentFolder' && !data.alias && $.bcTree.settings[data.contentType]['routes']['copy'] != undefined && mode == 'index') {
							$.extend(true, menu, {
								"copy": {
									label: "コピー",
	 ...
	 */
		createContent: function (parent, data) {
			...
			if ((!$.bcTree.settings[data.contentType]['multiple'] && $.bcTree.settings[data.contentType]['exists']) || data.contentAliasId) {
				...
				url = $.bcTree.settings[data.contentType]['routes']['add']
			}
	 ...
	 */
		copyContent: function (parent, node) {
			...
			data.status = "0";
			$.ajax({
				url: $.bcTree.settings[data.contentType]['routes']['copy'],
				type: 'POST',
				data: {
					contentId: data.contentId,
					entityId: data.contentEntityId,
