...
<?php if(count($sites) >= 2): ?>
    <small><?php echo __d('baser', 'サイト') ?></small> <?php echo $this->BcForm->input('ViewSetting.site_id', array('type' => 'select', 'options' => $sites)) ?>　｜　
<?php else : ?>
    <?php echo $this->BcForm->input('ViewSetting.site_id', array('type' => 'hidden', 'options' => $sites)) ?>
<?php endif ?>
    <small><?php echo __d('baser', '表示設定') ?></small> <?php echo $this->BcForm->input('ViewSetting.list_type', array('type' => 'radio', 'options' => $listTypes)) ?>
