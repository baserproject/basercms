...
<ul>
	...
	<li><?php echo sprintf(__d('baser', '一覧左上の「並び替え」をクリックすると、各データの操作欄に表示される %s マークをドラッグアンドドロップして、公開ページにおけるデータの表示順の変更を行う事ができます。<br />
		<small>※ この並び順はウィジェットのローカルナビゲーション等に反映されます。'), $this->BcBaser->img('admin/sort.png', ['alt' => __d('baser', '並び替え')])) ?></small></li>
	<li><?php echo __d('baser', 'オーサリングツールでの制作に慣れている方向けに、ファイルをアップロードしてデータベースに一括で読み込む機能を備えています。<br />
...
<div class="example-box">
	...
	<pre><?php echo __d('baser', 'http://{baserCMS設置URL}/about')?></pre>
	<p class="head"><?php echo __d('baser', '（例）カテゴリ名「company」に属する、ページ名「about」として作成したページを表示させる為のURL')?></p>
	<pre><?php echo __d('baser', 'http://{baserCMS設置URL}/company/about')?></pre>
</div>