...
<html lang="ja">
	<head>
		...
		<?php $this->BcBaser->css(['admin/html5reset-1.6.1', 'smartphone/style']) ?>
		<?php $this->BcBaser->js([
				'jquery-1.7.2.min',
...
<?php $this->BcBaser->googleAnalytics([], ['subDir' => false]) ?>
	...
	<body id="<?php $this->BcBaser->contentsName() ?>">
		<div id="Page">
			...
			<header>
				<div class="clearfix" id="BoxLogo">
					<div id="Logo"><h1><?php $this->BcBaser->logo(['link' => '/s/']) ?></h1></div>
				</div>
		...
		<?php if ($this->BcBaser->isHome()): ?>
			<div id="top-main">
			<?php $this->BcBaser->mainImage(['all' => true, 'num' => 5, 'link' => '']) ?>
			</div>
			<?php
			/*
		...
		<?php endif; ?>
			...
			<?php if(!empty($widgetArea)): ?>
				<?php $this->BcBaser->element('widget_area', ['no' => $widgetArea]); ?>
			<?php endif; ?>
			</div>
