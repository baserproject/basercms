 ...
 */
	function uploadSuccessHandler(res){
		if(res === '1'){
			...
			var message = bcI18n.uploaderAlertMessage2;
			// バリデーションエラーメッセージを動的に取得・表示するため
			var response = jQuery.parseJSON(res);
			$.each(response, function (fieldName, validationErrorList) {
				$.each(validationErrorList, function(key, validationErrorMessage) {
					message = validationErrorMessage;
				});
			});

			$('#ErrorMessage').remove();
			$('#FileList'+listId).prepend('<p id="ErrorMessage" class="message">' + message + '</p>');
			$("#Waiting").hide();
		}
		// フォームを初期化
