...
<?php if(!empty($uploaderCategories)): ?>
	<small style="font-weight:bold"><?php echo __d('baser', 'カテゴリ')?></small>&nbsp;<?php echo $this->BcForm->input('Filter.uploader_category_id', array('type' => 'select', 'options' => $uploaderCategories, 'empty' => '指定なし', 'id' => 'FilterUploaderCategoryId'.$listId, 'class' => 'filter-control')) ?>　
<?php endif ?>
	<small style="font-weight:bold"><?php echo __d('baser', 'タイプ')?></small>&nbsp;<?php echo $this->BcForm->input('Filter.uploader_type', array('type' => 'radio', 'options' => array('all'=>'指定なし', 'img' => '画像', 'etc' => '画像以外'), 'id' => 'FilterUploaderType'.$listId, 'class' => 'filter-control')) ?>
