			...
			<?php if (!$blockEnd || $iteration <= $blockEnd): ?>
				...
				<?php if (($this->BcArray->last($mailFields, $key)) ||
					...
					<?php if ($field['group_valid']): ?>
						...
						<?php endif; ?>
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $field['group_valid'] . "_not_same", __("入力データが一致していません"), array('wrap' => false)) ?></font>
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $field['group_valid'] . "_not_complate", __("入力データが不完全です"), array('wrap' => false)) ?></font>
