		...
		<?php if ($blockStart && $iteration >= $blockStart): ?>
			<?php if (!$blockEnd || $iteration <= $blockEnd): ?>
				...
				<?php if ($group_field != $record['MailField']['group_field'] || (!$group_field && !$record['MailField']['group_field'])): ?>
					<br />
					<br />
					<span style="color:#8ABE08">■</span> <?php echo $this->Mailform->label("MailMessage." . $record['MailField']['field_name'] . "", $record['MailField']['head']) ?>
				...
				<?php if (!$record['MailField']['group_valid']): ?>
					<?php if ($this->Mailform->error("MailMessage." . $record['MailField']['field_name'] . "_format", "check")): ?>
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $record['MailField']['field_name'] . "_format", "形式が不正です", ['wrap' => false]); ?></font>
					<?php else: ?>
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $record['MailField']['field_name'] . "", "必須項目です", ['wrap' => false]); ?></font>
					<?php endif; ?>
				<?php endif; ?>
				<?php /* 説明欄 */ ?>
				<?php if (($this->BcArray->last($mailFields, $key)) ||
					...
					<?php if ($record['MailField']['group_valid']): ?>
						<?php if ($this->Mailform->error("MailMessage." . $record['MailField']['group_field'] . "_format", "check")): ?>
							<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $record['MailField']['group_field'] . "_format", "形式が不正です", ['wrap' => false]) ?></font>
						<?php else: ?>
							<?php if ($record['MailField']['valid']) : ?>
								<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $record['MailField']['group_field'] . "", "必須項目です", ['wrap' => false]) ?></font>
							<?php endif; ?>
						<?php endif; ?>
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $record['MailField']['group_field'] . "_not_same", "入力データが一致していません", ['wrap' => false]) ?></font> 
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $record['MailField']['group_field'] . "_not_complate", "入力データが不完全です", ['wrap' => false]) ?></font>
					<?php endif; ?>
				<?php endif; ?>
