			...
			<?php if (!$blockEnd || $iteration <= $blockEnd): ?>
				...
				<?php if (!$field['group_valid']): ?>
					<?php if ($this->Mailform->error("MailMessage." . $field['field_name'] . "_format", "check")): ?>
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $field['field_name'] . "_format", "形式が不正です", array('wrap' => false)); ?></font>
					<?php else: ?>
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $field['field_name'] . "", "必須項目です", array('wrap' => false)); ?></font>
					<?php endif; ?>
				<?php endif; ?>
				<?php /* 説明欄 */ ?>
				<?php if (($this->BcArray->last($mailFields, $key)) ||
					...
					<?php if ($field['group_valid']): ?>
						<?php if ($this->Mailform->error("MailMessage." . $field['group_field'] . "_format", "check")): ?>
							<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $field['group_field'] . "_format", "形式が不正です", array('wrap' => false)) ?></font>
						<?php else: ?>
							<?php if ($field['valid']) : ?>
								<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $field['group_field'] . "", "必須項目です", array('wrap' => false)) ?></font>
							<?php endif; ?>
						<?php endif; ?>
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $field['group_field'] . "_not_same", "入力データが一致していません", array('wrap' => false)) ?></font> 
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $field['group_field'] . "_not_complate", "入力データが不完全です", array('wrap' => false)) ?></font>
					<?php endif; ?>
				<?php endif; ?>
