...
1. **ルート `tests/bootstrap.php` の `Migrator::runMany`**（初期スキーマ構築）
   ...
   ```
2. **`plugins/baser-core/tests/TestCase/Service/BcDatabaseServiceTest.php` の `test_deleteTablesForMigrations()` の `$plugins` 配列**（再構築用）。`test_deleteTables` が `deleteTables()` で**全テーブルを drop** した後、このリストで再マイグレーションする。ここに無いとテーブルが復活せず、テスト順で BaserCore が先頭のため**以降の全テストで欠落したまま**になる（実際に bc-mcp 追加時に `oauth2_clients` がこれで消え、OAuth2 全テストが 500 になった）。
   ```php
   $plugins = [ 'BaserCore', ..., 'BcMail', 'BcMcp', /* 追加 */ 'BcSearchIndex', ... ];
   ```
- **抜け漏れ確認**: コアプラグインを列挙している箇所は他にもあり得る。`grep -rn "'BcMail'," plugins/baser-core/tests plugins/baser-core/config tests` 等で同種リストを洗い、新プラグインが全リストに入っているか確認する。
